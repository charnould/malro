...
export const view = async (data: Event[], options: Options) => html`
  ...
  <html lang=${options.lang}>
    ...
    <head>
      ...
      <meta name="description" content="${data[0].description[0].title} (${data[0].description[0].organizer})${
				data.length >= 2 ? '...' : null
			}" />
      <meta property="og:description" content="${data[0].description[0].title} (${data[0].description[0].organizer})${
				data.length >= 2 ? '...' : null
			}" />
    ...
    <body ${options.embed === true ? null : 'class=padded'}>
      ...
      <div class="card">
        ...
        <div class="content">${
					e.status === 'trashed'
						? html`<p class="danger">${await translate('trash', 'widget', options.lang)}</p>`
						: null
				}
          ...
          <p class="tags">${await Promise.all(
						e.type.map(async (t) => html`<span>${await translate(t, 'enums', options.lang)}</span>`)
					)}</p>
        
          <div class="calendar-table">
          ${await Promise.all(
						e.calendar.slice(0, 5).map(
							async (ts) => html`
          <a class="calendar-row" href="/ical/${e.id}/${ts.index}" download="event">
            ...
            <span>${new Intl.DateTimeFormat(options.lang, { month: 'short', day: '2-digit' }).format(
							new Date(ts.start)
						)}</span>
            <span>${new Intl.DateTimeFormat(options.lang, { minute: '2-digit', hour: '2-digit', hour12: false }).format(
							new Date(ts.start)
						)} &rarr; ${new Intl.DateTimeFormat(options.lang, {
							minute: '2-digit',
							hour: '2-digit',
							hour12: false
						}).format(new Date(ts.end))}</span>
            <span>€</span>
        ...
        <div class="footer">
          <p><a href="${e.description[0].url}/?utm_source=malro.org" target="_blank">${await translate(
						'info',
						'widget',
						options.lang
					)}</a> &nbsp; <a href="${env.BASE_URL}/sql/select%20*%20from%20events%20where%20id='${
						e.id
					}'" target="_blank">${await translate('share', 'widget', options.lang)}</a></p>
          <p><a href="${env.BASE_URL}/sql/select%20*%20from%20events%20where%20id='${
						e.id
					}'?format=json" target="_blank">Open data</a> by <a href="${env.BASE_URL}?utm_source=malro_widget" target="_blank">MALRO</a></p>
        </div>
      </div>`
				)
			)}
